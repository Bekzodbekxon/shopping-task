...
export function listenToProductList({commit},pagination) {

    ApiService.getProducts(pagination).then(response => {
...
export function getByCategory({commit},category){
    ApiService.getByCategory(category).then(response=>{
        if (response){
            commit('UPDATE_PRODUCT_LIST',response.data)
        }
    })
